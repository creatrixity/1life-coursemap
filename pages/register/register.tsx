...
export class RegisterPage extends React.Component<IHomePage.IProps, IHomePage.IState> {
	state = {
		name: '',
		email: ''
	}
			...
			<div className="d-flex flex-row align-items-center auth-screen auth-screen--login">
				...
				<section className={'d-flex flex-column w-100 justify-content-center mb-5 py-5 px-3'}>
					<div className={'auth-screen--header text-center mb-4'}>
						...
						<h1 className={'auth-screen__title font-weight-bold mb-3 h3'}>Register</h1>
					</div>
					<form onSubmit={this.onRegisterFormSubmit}>
						<div className={'form-group mb-4'}>
							<label>Full name</label>
							<input type={'text'} required value={this.state.name} className={'form-control mb-2'} onChange={e => this.setState({ name: e.target.value })} />
						</div>

						<div className={'form-group mb-4'}>
							<label>Your E-mail</label>
							<input type={'email'} required value={this.state.email} className={'form-control mb-2'} onChange={e => this.setState({ email: e.target.value })} />
						</div>
						...
						<div className={'form-group mb-4'}>
							...
							<p className={'text-danger my-2'}>{ user.registrationErrorMessage }</p>
						</div>
