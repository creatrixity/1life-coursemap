...
import { AppActions, ModuleActions } from '@Actions';
...
> {
  ...
  componentDidMount() {
    ...
    fetchCourseModuleLessonView(
      { courseSlug, moduleSlug, lessonId: 0 },
    ...
    getUserLessons(
      ...
      ({ userLessons, moduleLessonsCount }: any) => {
        moduleProgression = computeModulesProgression(
          userLessons,
          moduleLessonsCount
        );
        ...
        this.setState({
          moduleProgression,
          userLessons: userLessons.map(
            (userLesson: any) => userLesson.lesson_id
          )
        });
      }
    );
  ...
  renderModuleCardLink({
    ...
    return (
      <div
        className={`card d-flex flex-row module__link-card ${isDisabled &&
          'module__link-card--disabled'}`}
      >
        <section
          className={
          ...
          <ul className={'list list-unstyled'}>
            {lessons &&
              lessons.map(({ id, title, type }: any, key: number) => {
                ...
                const isUserLesson =
                  this.state.userLessons.includes(id) || key === 0;
                const icon = isLesson ? PlayIcon : ToolIcon;
                const WrapperElement = isUserLesson ? Link : 'div';
                const WrapperProps = isUserLesson
                  ? {
                      href: `/lesson?course=${course}&module=${module}&lesson=lesson-${id}`,
                      as: `/lesson/${course}/${module}/lesson-${id}`
                    }
                  : {};
  ...
  public render(): JSX.Element {
    ...
    return (
      ...
      >
        <Head>
          <title>{toTitleCase(moduleTitle)} | 1Life</title>
        </Head>

        <Header
          backlinkHref={{
            link: `/course?course=${course}`,
            as: ``
          }}
          title={moduleTitle}
        >
          {!app.isUpdatingUserLesson ? (
            <StarsCast progression={moduleProgression || 0} />
          ) : (
          ...
          {isFetchingCourseModuleLessonView ? (
            ...
            <div className={'container'}>
              ...
              <section className={'row'}>
                <div className={'col-12'}>{this.renderUpcomingLinks()}</div>
              </section>
            </div>
          )}
        </section>
...
const mapStateToProps = (state: IStore) => {
  return {
    ...
    isFetchingCourseModuleLessonView: ModuleSelectors.getIsFetchingCourseModuleLessonView(
      state
    ),
    isFetchingCourseModuleLessons: ModuleSelectors.getIsFetchingCourseModuleLessons(
      state
    )
  };
};
...
const mapDispatchToProps = (dispatch: Dispatch) => ({
  fetchCourseModuleLessonView: bindActionCreators(
    ModuleActions.fetchCourseModuleLessonView,
    dispatch
  ),
